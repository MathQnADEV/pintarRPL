...
<head>
    ...
    <title>PINTAR - Hasil Kuis: {{ $review['topic_title'] }}</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])

    {{-- Highlight.js — untuk blok kode di teks soal & penjelasan --}}
    <link rel="stylesheet"
...
<body class="bg-[#f7f7f7] font-sans text-gray-800 min-h-screen">
    ...
    <nav class="bg-white border-b-2 border-gray-100 px-6 py-4">
        <div class="max-w-3xl mx-auto flex items-center justify-between">
            ...
            <span class="text-sm text-gray-400 font-semibold">{{ $review['topic_title'] }}</span>
        </div>
    </nav>
    ...
    <main class="max-w-3xl mx-auto px-4 py-8 space-y-6">
        ...
        <div class="rounded-3xl border-2 border-b-4 overflow-hidden
            ...
            <div class="px-8 py-10 text-center
                        {{ $passed ? 'bg-[#58cc02]' : 'bg-[#ff4b4b]' }}">
                <div class="text-5xl mb-3">{{ $passed ? '🏆' : '😓' }}</div>
                <h1 class="text-white font-extrabold text-2xl tracking-tight mb-1">
                    {{ $passed ? 'Luar Biasa! Kuis Selesai!' : 'Belum Berhasil — Coba Lagi!' }}
                </h1>
                <p class="text-white/80 text-sm">
                    {{ $passed
                        ? 'Kamu berhasil melewati batas nilai kelulusan.'
                        : 'Nilai minimum kelulusan adalah 60. Kamu pasti bisa!' }}
                </p>
            </div>
        ...
        <div class="flex gap-3 flex-wrap">
            <a href="{{ route('mahasiswa.kuis', $topic) }}"
               class="flex-1 min-w-[130px] text-center bg-white text-[#4c3fb5] font-extrabold py-4 px-4 rounded-2xl
                      border-2 border-b-4 border-[#4c3fb5] uppercase tracking-wide text-xs
                      hover:bg-[#f0effe] active:translate-y-1 active:border-b-2 transition-all">
                🔄 Ulangi Kuis
            </a>
            <a href="{{ route('mahasiswa.review', $topic) }}"
               class="flex-1 min-w-[130px] text-center bg-white text-[#1cb0f6] font-extrabold py-4 px-4 rounded-2xl
                      border-2 border-b-4 border-[#1cb0f6] uppercase tracking-wide text-xs
        ...
        {{-- ─── Review tiap soal ────────────────────────────────────────────── --}}
        <h2 class="font-extrabold text-lg text-gray-700 pt-2">
            📋 Review Jawaban
        </h2>

        @foreach ($review['questions'] as $qi => $q)
            @php $isCorrect = $q['is_correct']; @endphp
            ...
            <div class="bg-white rounded-3xl border-2 border-b-4 overflow-hidden
                ...
                <div class="px-6 pt-5 pb-3 flex items-start gap-3">
                    <span class="w-9 h-9 rounded-xl shrink-0 flex items-center justify-center font-extrabold text-sm
                                 {{ $isCorrect ? 'bg-[#d7f0bc] text-[#4a8c00]' : 'bg-[#fde8e8] text-[#cc2929]' }}">
                        {{ $isCorrect ? '✓' : '✗' }}
                    </span>
                    <div class="question-body flex-1 text-gray-800 text-sm leading-relaxed font-semibold">
                        <span class="text-xs font-extrabold uppercase tracking-widest text-gray-400 mr-2">
                            Soal {{ $qi + 1 }}
                            @if (($q['type'] ?? 'multiple_choice') === 'code_arrange')
                ...
                @if (($q['type'] ?? 'multiple_choice') === 'code_arrange')
                    ...
                    <div class="px-6 pb-4 space-y-4">
                        {{-- Jawaban mahasiswa --}}
                        <div>
                            <p class="text-xs font-extrabold uppercase tracking-widest mb-2
                                       {{ $isCorrect ? 'text-[#3a7200]' : 'text-[#cc2929]' }}">
                                Susunanmu
                            </p>
                            <div class="flex flex-wrap gap-1">
                                @forelse ($userOrder as $idx => $uid)
                                    @php $isRight = isset($correctOrder[$idx]) && $correctOrder[$idx] == $uid; @endphp
                                    <span class="review-tile {{ $isRight ? 'tile-correct' : 'tile-wrong' }}">
                                        <span class="tile-numbadge">{{ $idx + 1 }}</span>{{ $tiles[$uid] ?? '?' }}
                                    </span>
                                @empty
                        ...
                        @if (! $isCorrect)
                            <div>
                                <p class="text-xs font-extrabold uppercase tracking-widest text-[#3a7200] mb-2">Urutan Benar</p>
                                <div class="flex flex-wrap gap-1">
                                    @foreach ($correctOrder as $idx => $cid)
                                        <span class="review-tile tile-correct">
                                            <span class="tile-numbadge">{{ $idx + 1 }}</span>{{ $tiles[$cid] ?? '?' }}
                                        </span>
                                    @endforeach
                                </div>
                            </div>
                        ...
                        @if (! empty($distractors))
                            <div>
                                <p class="text-xs font-extrabold uppercase tracking-widest text-gray-400 mb-2">
                                    Tile Pengecoh <span class="font-normal normal-case">(seharusnya TIDAK dipilih)</span>
                                </p>
                                <div class="flex flex-wrap gap-1">
                                    @foreach ($distractors as $did => $dtext)
                                        <span class="review-tile tile-neutral opacity-60">
                                            🚫 {{ $dtext }}
                                        </span>
                                    @endforeach
                                </div>
                ...
                @else
                    {{-- Multiple choice --}}
                    @php $selectedId = $q['selected_id'] ?? null; @endphp
                    <div class="px-6 pb-4 space-y-2">
                        @foreach ($q['options'] as $opt)
                            @php
                                $isOptCorrect  = $opt['is_correct'];
                                $isOptSelected = $opt['id'] === $selectedId;
                            ...
                            <div class="flex items-center gap-3 p-3 rounded-xl border-2 {{ $optBg }}">
                                <span class="w-8 h-8 rounded-lg shrink-0 flex items-center justify-center
                                             font-extrabold text-xs {{ $iconColor }}">{{ $optIcon }}</span>
                ...
                @if (! empty($q['explanation']))
                    <div class="mx-6 mb-5 p-4 bg-[#fffbeb] border-l-4 border-[#f59e0b] rounded-xl">
                        <p class="text-xs font-extrabold uppercase tracking-widest text-[#b45309] mb-2">💡 Penjelasan</p>
                        <div class="explanation-body text-sm text-[#78350f] leading-relaxed">{!! $q['explanation'] !!}</div>
                    </div>
                @endif
            </div>
        @endforeach

        {{-- ─── CTA Bawah (duplikat untuk UX) ─────────────────────────────── --}}
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            // Wrap bare <pre> (output Trix) menjadi <pre><code>
